---
name: godot-gdscript
description: GDScript 4.x syntax reference — types, operators, classes, signals, patterns, and common API. Load when writing or reviewing GDScript code.
---
...
process_mode = Node.PROCESS_MODE_ALWAYS    # Exempt node from pause
...
# State machine manages current + stack for push/pop:
...
# 3D: same pattern with NavigationAgent3D
...
draw_set_transform(pos, rotation, scale)  # stateful
...
- Grid alignment assist: snap Y to nearest row center when moving horizontally (`round(pos.y / tile_size) * tile_size + tile_size / 2`), vice versa. Prevents snagging on corridor entrances.
...
- **Detach child camera:** Set `top_level = true` in `_ready()` on a Camera3D that's a child of a moving node.
...
- **Camera-relative input:** Remove pitch from camera basis, multiply input by it:
  ```gdscript
  ...
  ```
- **Dynamic FOV:** `camera.fov = clamp(base_fov + (speed - threshold) * factor, base_fov, max_fov)`
