...
_SAFE_AGENT_FILENAME = re.compile(r"[^A-Za-z0-9._-]+")
...
def _resolve_harness_prompt_target_dir(
    *, harness: str, global_scope: bool, project: Path | None, version: str
) -> Path:
    if harness == "opencode":
        if global_scope:
            return Path.home() / ".config" / "opencode" / "agents"
        assert project is not None
        return project.resolve() / ".opencode" / "agents"

    if harness == "pi":
        if global_scope:
            return Path.home() / ".pi" / "agent" / "agents"
        assert project is not None
        return project.resolve() / ".pi" / "agents"

    if global_scope:
        prompts_base_dir = get_global_config_base_dir() / "prompts"
    else:
        prompts_base_dir = get_project_config_base_dir(project) / "prompts"
    return prompts_base_dir / harness / version


def _load_harnessctl_agent_registry() -> list[dict[str, object]]:
    ...
    if not config_path.is_file():
        typer.secho(
            f"Harnessctl config path is not a regular file: {config_path}",
            fg=typer.colors.RED,
        )
        raise typer.Exit(code=2)
    ...
    try:
        ...
        typer.secho(f"Failed to read harnessctl config: {exc}", fg=typer.colors.RED)
        raise typer.Exit(code=2) from exc

    if not isinstance(loaded, dict):
        return []
    spec = loaded.get("spec")
    if not isinstance(spec, dict):
        return []
...
def _agent_filename(agent_id: str) -> str:
    filename = _SAFE_AGENT_FILENAME.sub("-", agent_id.strip().lower()).strip("-.")
    if not filename:
        filename = "worker"
    return f"{filename}.md"


def _render_opencode_agent_template(*, description: str, model: str) -> str:
    return (
        "---\n"
...
def _render_pi_agent_template(*, name: str, description: str, model: str) -> str:
    return (
        "---\n"
        f"name: {name}\n"
        f'description: "{description}"\n'
        f"model: {model}\n"
        "---\n\n"
        f"You are {name}. Execute delegated tasks carefully and report clear outcomes.\n"
    )
...
def prompts_install_command(
    ...
    """Install prompt bundles into harness-native paths."""
    if global_scope == (project is not None):
        typer.secho(
            "You must specify exactly one target: --global or --project <path>",
            fg=typer.colors.RED,
    ...
    if "opencode" in selected_harnesses or "pi" in selected_harnesses:
        registry_agents = _load_harnessctl_agent_registry()

    for harness_name in selected_harnesses:
        target_dir = _resolve_harness_prompt_target_dir(
        ...
        if harness_name == "opencode":
            target_files.append(
                (
                    ...
                    _render_opencode_agent_template(
            ...
            for agent in registry_agents:
                agent_id = str(agent.get("id") or "worker").strip() or "worker"
                model = str(agent.get("model") or "github-copilot/gpt-5-mini").strip()
                if not model:
                    model = "github-copilot/gpt-5-mini"
                target_files.append(
                    (
                        harness_name,
                        target_dir / _agent_filename(agent_id),
                        _render_opencode_agent_template(
                            description=f"{agent_id} - Generated by harnessctl",
                            model=model,
                        ),
                    )
                )
        ...
        if harness_name == "pi":
            ...
            for agent in registry_agents:
                ...
                if not model:
                    model = "github-copilot/gpt-5-mini"
                safe_name = (
                    _SAFE_AGENT_FILENAME.sub("-", agent_id).strip("-.") or "worker"
                )
                target_files.append(
                    (
                        harness_name,
    ...
    existing = [str(path) for _, path, _ in target_files if path.exists()]
    if existing and not overwrite:
        typer.secho(
            "Refusing to overwrite existing files (use --overwrite):\n"
            + "\n".join(f"- {path}" for path in existing),
    ...
    for _, output_path, content in target_files:
